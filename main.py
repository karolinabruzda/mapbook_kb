...
def update_user(i) -> None:
    users[i].imie = imie = entry_imie.get()
    users[i].nazwisko = nazwisko = entry_nazwisko.get()
    users[i].postow = postow = entry_liczba_postow.get()
...
label_szczegoly_obiektow = Label(ramka_szczegoly_obiektow, text='Szczegoly obiektow: ')
label_szczegoly_obiektow.grid(row=0, column=0, sticky=W)
label_szczegoly_imie = Label(ramka_szczegoly_obiektow, text='Imie: ')
...
map_widget.set_position(deg_x=52.0, deg_y=21.0)
